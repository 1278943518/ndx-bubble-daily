...
def download_historical_data():
    for date in range(2011, 2021):
        source = "https://www.ici.org/system/files/%s/flows_data_%s.xls" % (date, date)
...
def download_present_data():
    base_url = (
        "https://www.ici.org/system/files/{year}-{month:02d}/etf_flows_data_{year}.xls"
    )

    years = range(2021, datetime.utcnow().year + 1)
    months = range(1, 13)
...
def extract_2016_2020(file):
    ...
    monthly_start = df[
        df.iloc[:, 0].str.lower().str.contains("monthly", na=False)
    ].index[0]
    weekly_start = df[df.iloc[:, 0].str.lower().str.contains("weekly", na=False)].index[
        0
    ]
    ...
    monthly_data.iloc[:, 0] = pd.to_datetime(
        monthly_data.iloc[:, 0], errors="coerce"
    ).dt.strftime("%Y-%m-%d")
    monthly_data.dropna(subset=[monthly_data.columns[0]], inplace=True, how="all")
    weekly_data.iloc[:, 0] = pd.to_datetime(
        weekly_data.iloc[:, 0], errors="coerce"
    ).dt.strftime("%Y-%m-%d")
